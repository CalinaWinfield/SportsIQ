...
export const db = {
    ...
    getLeaderboard: (league, mode, limit = 50) => {
    ...
    addFavorite: (fav) => {
        const exists = inMemoryDb.favorites.some(f => f.userId === fav.userId && f.league === fav.league && f.teamId === fav.teamId);
        if (!exists) {
    ...
    removeFavorite: (userId, league, teamId) => {
        const initialLen = inMemoryDb.favorites.length;
        inMemoryDb.favorites = inMemoryDb.favorites.filter(f => !(f.userId === userId && f.league === league && f.teamId === teamId));
